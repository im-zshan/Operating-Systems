// Run Using "gcc -pthread 29_producer_consumer_multi_semaphores_unfairness.c && ./a.out"
...
void* producer(sem_t* empty, sem_t* full, sem_t* buf_lock, int* buffer, int* head, int* tail, int data, int id) {
    sem_wait(empty);
    sem_wait(buf_lock);
...
int consume(sem_t* empty, sem_t* full, sem_t* buf_lock, int* buffer, int* head, int* tail, int id) {
    sem_wait(full);
    sem_wait(buf_lock);
    if (id == 0) usleep(500); // Makes all threads suffer
...
void* producer_thread(void* vargp) {
    ...
    *t = clock();
    for (int i = 0; i < OPR; i++) {
        ...
        *num = *num + data;
        sem_post(num_lock);
        producer(empty, full, buf_lock, buffer, head, tail, data, id);
        // usleep(100);
    }
    *t = clock() - *t;
...
void* consumer_thread(void* vargp) {
    ...
    *t = clock();
    for (int i = 0; i < OPR; i++) {
        int data = consume(empty, full, buf_lock, buffer, head, tail, id);
        sem_wait(num_lock);
        // printf("num = %d - %d = %d\n", *num, data, *num - data);
        *num = *num - data;
        sem_post(num_lock);
